...
function calcPrices(orderItems) {
   ...
   const totalPrice = (parseFloat(itemsPrice) + parseFloat(taxPrice)).toFixed(2);
...
const createOrder = async (req, res) => {
   try {
      ...
      if (!orderItems || !Array.isArray(orderItems) || orderItems.length === 0) {
         res.status(400).json({ error: "No order items provided or invalid format" });
         return;
      }
      ...
      const dbOrderItems = orderItems.map((itemFromClient) => {
         ...
         if (!matchingItemFromDB) {
            res.status(404).json({ error: `Product not found: ${itemFromClient._id}` });
